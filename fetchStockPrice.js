...
async function complexOperations() {
  let factResult = await calculateFactorial(5);
  console.log(`Factorial of 5: ${factResult}`);

  let isPrimeResult = await isPrime(13);
  console.log(`Is 13 prime? ${isPrimeResult}`);

  let randomArray = await generateRandomArray(10, 1, 100);
  console.log(`Random array: ${randomArray}`);

  let mergedArray = await mergeSortedArrays([1, 3, 5], [2, 4, 6]);
  console.log(`Merged array: ${mergedArray}`);

  let longestString = await findLongestString(['apple', 'banana', 'kiwi']);
  console.log(`Longest string: ${longestString}`);

  let flattenedArray = await flattenArray([1, [2, 3], [4, [5, 6]]]);
  console.log(`Flattened array: ${flattenedArray}`);

  let reversedString = await reverseWordsInString('hello world');
  ...
  let matrixSum = await calculateMatrixSum([[1, 2], [3, 4]]);
  console.log(`Matrix sum: ${matrixSum}`);

  let clonedObject = await deepClone({ name: 'John', age: 30 });
